...
namespace FinalGame
{
    ...
    /// Admittedly, this is pretty much copied from the Windows XNAGame from Unit 12. ...Sorry.
    /// The only reason this is is because I needed something to get debug messages from
    /// quickly and reasonably efficiently.
    /// However, the getMsg method from the Windows game was removed since getting resources from
    /// a phone is more tricky, and almost everything needs to come from the ContentManager.
    /// </summary>
    public static class Text
    {
        ...
        /// </summary>
        public static SpriteFont font = GameState.loader.Load<SpriteFont>("fntMain");
            ...
            for (int i = 0; i < msg.Length; i++)
            {
                if (msg[i] == '<')
                {
                    ...
                    else if (commands[0].Equals("scale", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        curX += getWidth(curStr);
                        curStr = "";
                        thisScale = float.Parse(commands[1]);
                    }
                    else if (commands[0].Equals("/scale", StringComparison.OrdinalIgnoreCase))
                    {
                        messages.Add(new float[8] { curX, curY, thisR, thisG, thisB, thisA, thisScale, curEffects }, curStr);
